...
async def handle_user_approval_action_from_task_cb(callback_query: CallbackQuery, bot: Bot):
    if not callback_query.from_user or not await is_admin(callback_query.from_user.id,
                                                          bot) or not callback_query.message:
    ...
    if not target_user_db_info:
        ...
        await db.update_request_status(task_request_id, "error_user_not_found",
                                       admin_note="Target user for approval task not found.")
        try:
            await callback_query.message.edit_text(
                f"Error: Target user ID {target_user_id} not found for task {task_request_id}\. Task closed due to error\.",
                reply_markup=None)
        except Exception as e:
    ...
    try:
        ...
        logger.warning("first_name or username not found for user %s in db_info via key access, using id.",
                       target_user_id)
    ...
    if action == CALLBACK_USER_APPROVAL_TASK_APPROVE:
        if current_target_user_status == USER_STATUS_APPROVED:
            action_result_text = MSG_USER_ALREADY_APPROVED_EDIT.format(user_name=target_user_name,
                                                                       user_id=target_user_id, task_id=task_request_id)
            await callback_query.answer(MSG_USER_ALREADY_APPROVED_ALERT, show_alert=True)
            await db.update_request_status(task_request_id, new_task_status,
        ...
        action_result_text = MSG_ADMIN_USER_APPROVED_CONFIRM.format(user_name=target_user_name, user_id=target_user_id,
                                                                    task_id=task_request_id)
    elif action == CALLBACK_USER_APPROVAL_TASK_REJECT:
        if current_target_user_status == USER_STATUS_REJECTED:
            action_result_text = MSG_USER_ALREADY_REJECTED_EDIT.format(user_name=target_user_name,
                                                                       user_id=target_user_id, task_id=task_request_id)
        ...
        action_result_text = MSG_ADMIN_USER_REJECTED_CONFIRM.format(user_name=target_user_name, user_id=target_user_id,
                                                                    task_id=task_request_id)
    else:
    ...
    await db.update_request_status(task_request_id, new_task_status,
                                   admin_note=f"Action taken by admin {admin_user_id}: {action}")
    await db.log_admin_action(admin_user_id, log_action_key,
    ...
    try:
        ...
        logger.error("keyerror: 'chat_id' column unexpectedly missing for user %s from db.get_user result.",
                     target_user_id)
    ...
    if chat_id_to_notify:
        try:
            ...
            logger.info("successfully sent '%s' notification to user %s (chat_id: %s)", action, target_user_id,
                        chat_id_to_notify)
        except Exception as e:
            logger.error("failed to notify user %s (chat_id: %s) about %s: %s", target_user_id, chat_id_to_notify,
                         action, e)
    else:
        logger.warning("chat_id for target user %s is invalid or missing (%s), cannot send notification.",
                       target_user_id, chat_id_to_notify)
